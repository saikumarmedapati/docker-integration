...
const StatusForm: React.FC<StatusFormProps> = ({ editMode = false }) => {
  ...
  const loadStatus = async () => {
    try {
      ...
      const status = await statusApi.getById(parseInt(id!));
      setName(status.name);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editMode && id) {
        await statusApi.update({ id: parseInt(id), name });
      } else {
        await statusApi.create({ name });
      }
      navigate('/status');
    } catch (err: any) {
      setError(err.response?.data?.message || `Failed to ${editMode ? 'update' : 'create'} status`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <p className="text-gray-600">
            {editMode ? 'Update status information' : 'Create a new status for your assets'}
          </p>
        </div>
      </div>
